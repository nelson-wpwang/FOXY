...
if __name__ == "__main__":
    ...
    picture=[]
    path = os.listdir('/home/yihan/Downloads/photo/new_photo')
    path.sort()
    path.reverse()
    output_lst = list()
    for k in path:
        img_direct = '/home/yihan/Downloads/photo/new_photo/'+k
	...
	for i in picture:    
	    ...
	    a = 0
		    ...
		    x2 = int(j[2][0]) + int(j[2][2])//2
                    if x2 > whole_img.shape[0]
                        x2 = whole_img.shape[0] 
		    y2 = int(j[2][1]) + int(j[2][3])//2
                    if y2 > whole_img.shape[1]
                        y2 = whole_img.shape[1]
		    print x1,y1,x2,y2
